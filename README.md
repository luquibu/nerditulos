...
- **Delivery.** Attendees subscribe with Server-Sent Events to `/api/rooms/:slug/stream?lang=xx`. Event ids are cursors; a valid cursor resumes with a delta, anything else gets a fresh snapshot. The same `lang` can be original text in one session and a translation in the next; cursors carry the output type, so a cursor from the previous session receives a fresh snapshot. The window holds the last `PUBLIC_WINDOW_SEGMENTS` segments; there is no backward browsing. The reader page resolves its language from `?lang=es` or `?lang=en` in the URL, then the last choice saved in that browser, then the browser's language, and Spanish by default; that language sets both the interface and the caption stream. When the session does not offer it, the reader shows the session's default stream (Spanish when present, otherwise the first one) with a notice, and returns to the reader's language as soon as a session offers it.
