...
- `http://localhost:5173/` lists the rooms; `http://localhost:5173/r/sala-1` is a room's reader, and `http://localhost:5173/r/sala-1?lang=en` opens it in English (the language otherwise follows the choice saved in the browser, then the browser's language, then Spanish; the ES | EN switch in the header of the rooms list and of the console changes it for the whole interface, and the reader's caption selector does the same).
...
- In the console, prepare a session (title and source language), choose "Usar micrófono" or a WAV file, press "Probar micrófono" (or "Probar archivo") and speak to see the recognized text in the console without creating a session, then press "Iniciar". Open the room's reader in another window to follow the captions; a private window shows what an attendee without an account sees. The session offers its source language and the configured translation; the reader's menu lists what the session offers.
  The labels above are quoted in Spanish, the default interface language; with the switch set to EN the console shows their English equivalents.
